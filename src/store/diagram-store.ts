...
export const useDiagramStore = create<DiagramState>((set, get) => ({
  ...
  importProjectFile: async (file: File) => {
    ...
    set({
      ...
      viewMode: data.viewMode ?? 'module',
      showProductImages: (data.viewMode ?? 'module') === 'image',
      pages,
      activePageId: activePage.id,
      nodes: activePage.nodes,
      edges: activePage.edges,
      layerVisibility: data.layerVisibility ?? { audio: true, video: true, network: true, power: true, 'av-over-ip': true },
      focusedLayer: data.focusedLayer ?? null,
